...
os.makedirs("./data/temp_data/election_results", exist_ok=True)

# Download the zip file
response = requests.get(election_results_location)
if response.status_code == 200:
    zip_path = "./data/temp_data/election_results/election_data.zip"
    # Save zip file
    ...
    with zipfile.ZipFile(zip_path, "r") as zip_ref:
        zip_ref.extractall("./data/temp_data/election_results")
...
data_files = [
    f for f in os.listdir("./data/temp_data/election_results") if f.endswith(".csv")
]
...
for filename in data_files:
    if re.match("[0-9]+_General_Election_Returns.csv", filename):
        print(f"\nReading {filename}:")
        df = pd.read_csv(f"./data/temp_data/election_results/{filename}")
...
dataframe_to_bigquery(
    ...
    PROJECT_ID,
    OUT_DATASET_NAME,
    "historical_precinct_election_results",
    env=ENV,
