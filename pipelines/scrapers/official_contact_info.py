...
def read_senator_info_page(page_url):
    ...
    member_page_main_content = member_page_soup.find('div', class_='mainbody',id='main-content')
    # member_page_office_info = member_page_main_content.find_all("div",class_='onecolumn')[1]
    member_page_office_info = member_page_main_content.find('div',id='mem_addrs',class_='ui-corner-all')
    ...
    for td in member_page_office_info_table_row:
        ...
        for i in range(len(office_text)):
            ...
            if line == '(TEL)':
                ...
                continue
            if '(TEL)' in line or '(TEL)' in office_text[min(i+1, len(office_text)-1)]:
                phone = line.replace('(TEL)', '').strip()
            elif '(FAX)' in line or '(FAX)' in office_text[min(i+1, len(office_text)-1)]:
                fax = line.replace('(FAX)', '').strip()
            elif '(TOLL-FREE)' in line or '(TOLL-FREE)' in office_text[min(i+1, len(office_text)-1)]:
...
def get_senator_offices():
    ...
    for page in senate_pages:
        ...
        district = 'SD-' + page['page_url'].split('/')[-1].replace('.php','').replace('member?d=','')
...
def read_house_member_info_page(page_url):
    ...
    house_member_grid_container = house_member_soup.find('section',class_='grid-container')
...
if __name__ == '__main__':
    ...
    senate_addresses_df.to_csv('data/Contact Info/senate_official_contact_info.csv',sep='\t',index=False)
    house_member_addresses_df = get_house_offices()
    house_member_addresses_df.to_csv('data/Contact Info/house_official_contact_info.csv',sep='\t',index=False)

    all_member_addresses_df = pd.concat([senate_addresses_df, house_member_addresses_df])
    all_member_addresses_df.to_csv('data/Contact Info/all_member_official_contact_info.csv',sep='\t',index=False)