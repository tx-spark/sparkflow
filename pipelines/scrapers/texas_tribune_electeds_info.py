...
def get_texas_tribune_electeds_info():
    ...
    subdivisions = ['state.json','texas-house.json','texas-senate.json', 'us-house.json']
    ...
    for subdivision in subdivisions:
        ...
        if 'results' not in raw_data.keys():
            raise ConnectionError('Did not find expected data format on Texas Tribune API. Aborting.')
...
def clean_texas_tribune_electeds_info(raw_df):
    ...
    if len(missing_columns) > 0:
        ...
        raise Exception(f"The Data from Texas Tribune is missing the columns: {missing_columns_string}")
